...
### Изученные концепции (Часть 1 — FastAPI)

#### 1. **FastAPI и асинхронность**
- FastAPI построен на основе Starlette и поддерживает `async/await` из коробки.
- Обработчики маршрутов (endpoint functions) могут быть как обычными, так и `async def`.
- Автоматически генерирует OpenAPI-схему и интерактивную документацию по адресам `/docs` (Swagger UI) и `/redoc` (ReDoc).

#### 2. **Pydantic v2**
- Библиотека для валидации данных и сериализации на основе аннотаций Python.
- Модели наследуются от `BaseModel`; поля описываются с помощью `Field(...)` с ограничениями (`min_length`, `ge`, `pattern` и др.).
- `model_dump(exclude_unset=True)` — получение только явно переданных полей (используется в PATCH/PUT).
- `Config.from_attributes = True` — режим ORM для создания модели из атрибутов объекта.

#### 3. **APIRouter и структура проекта**
- `APIRouter` позволяет выносить эндпоинты в отдельные файлы и подключать их к основному приложению через `app.include_router(router, prefix=...)`.
- Разделение ответственности: `models.py` — схемы данных, `database.py` — хранилище, `routers.py` — бизнес-логика, `main.py` — точка входа.
- Вынос общего состояния в `database.py` решает проблему циклических импортов между `main.py` и `routers.py`.

#### 4. **HTTPException и статус-коды**
- `HTTPException(status_code=404, detail="...")` — стандартный способ вернуть ошибку клиенту.
- Используемые коды: `200 OK`, `201 Created`, `204 No Content`, `400 Bad Request`, `404 Not Found`, `409 Conflict`.

#### 5. **Enum и строгая типизация**
- `Genre(str, Enum)` — перечисление для жанров книг. FastAPI автоматически валидирует, что переданное значение является допустимым.
- Все функции снабжены аннотациями типов (`List[BookResponse]`, `Optional[str]`, `Counter`).

---

### Изученные концепции (Часть 2 — Express / Node.js)

#### 6. **Express и middleware-архитектура**
- Express — минималистичный веб-фреймворк для Node.js.
- **Middleware** — функции вида `(req, res, next)`, выполняющиеся по цепочке до обработчика маршрута. Используются для: безопасности (`helmet`), CORS, ограничения запросов (`express-rate-limit`), парсинга JSON, валидации.

#### 7. **Joi — валидация входных данных**
- Декларативные схемы (`Joi.object({...})`) для описания структуры запроса.
- Сравнение с Pydantic: Joi работает в рантайме и не требует TypeScript; Pydantic встроен в FastAPI и использует аннотации Python.
- Middleware `validateCreateTask` / `validateUpdateTask` вызывают `schema.validate(req.body)` и возвращают `400` при ошибке.

#### 8. **Файловая система как хранилище данных**
- `fs.promises` — асинхронный API Node.js для работы с файлами.
- `tasks.json` — простое JSON-хранилище (массив задач + счётчик ID).
- `readData()` / `writeData()` — атомарные операции чтения/записи всего файла.
- Преимущества для учебного проекта: не требует СУБД, данные сохраняются между перезапусками, легко читать вручную.

#### 9. **Порядок регистрации маршрутов в Express**
- Express сопоставляет маршруты в порядке их регистрации.
- `/stats/summary` и `/search/text` должны быть зарегистрированы **до** `/:id`, иначе Express интерпретирует `stats` и `search` как числовые идентификаторы и вернёт ошибку валидации.

#### 10. **async/await и обработка ошибок**
- Все обработчики маршрутов обёрнуты в `try/catch`; при ошибке вызывается `next(error)`, который передаёт управление централизованному `errorHandler`.
...
├── routers.py       # Все 8 эндпоинтов API
...
- [x] **Задача 1: Настройка проекта**
  - Созданы файлы `requirements.txt`, `models.py`, `database.py`, `main.py`
  - Настроен CORS и Swagger UI

- [x] **Задача 2: GET /books — список с фильтрацией и пагинацией**
  - Фильтрация по `genre` (точное совпадение с Enum), `author` (регистронезависимый поиск), `available_only` (булева)
  - Пагинация через срез списка: `filtered_books[skip : skip + limit]`

- [x] **Задача 3: GET /books/{id} — книга по ID**
  - `HTTPException(404)` при отсутствии книги
  - Если книга взята — в ответ добавляются поля `borrowed_by`, `borrowed_date`, `return_date` из `borrow_records`

- [x] **Задача 4: POST /books — создание книги**
  - Проверка уникальности ISBN: перебор `books_db.values()`, при дубликате — `HTTPException(409 Conflict)`
  - Сохранение в `books_db` с `available=True`

- [x] **Задача 5: PUT /books/{id} — обновление книги**
  - `HTTPException(404)` при отсутствии
  - Частичное обновление через `model_dump(exclude_unset=True)`
  - Проверка ISBN при его изменении

- [x] **Задача 6: DELETE /books/{id} — удаление книги**
  - `HTTPException(404)` при отсутствии
  - `HTTPException(400)` при попытке удалить взятую книгу (`available=False`)
  - Удаление из `books_db` и `borrow_records`

- [x] **Задача 7: POST /books/{id}/borrow — взять книгу**
  - Проверки: 404 (книга не найдена), 400 (книга уже взята)
  - Устанавливает `available=False`, создаёт запись в `borrow_records` с датами

- [x] **Задача 8: POST /books/{id}/return — вернуть книгу**
  - Проверки: 404 (книга не найдена), 400 (книга не взята)
  - Устанавливает `available=True`, удаляет запись из `borrow_records`

- [x] **Задача 9: GET /stats — статистика**
  - Общее количество книг, доступных, взятых
  - Распределение по жанрам через `Counter`
  - Самый плодовитый автор через `Counter.most_common(1)`
...
│   ├── routes/
│   │   └── tasks.js             # Все эндпоинты задач
│   ├── middleware/
│   │   ├── validation.js        # Joi-схемы и middleware валидации
│   │   └── errorHandler.js      # Обработчики 404 и общих ошибок
│   └── utils/
│       └── fileOperations.js    # Чтение/запись tasks.json
├── package.json
└── .gitignore
```
...
- [x] **Задача 1: Настройка проекта**
  - `package.json` с зависимостями: express, cors, helmet, joi, uuid, express-rate-limit, dotenv
  - `.gitignore` исключает `node_modules/`, `tasks.json`, `.env`

- [x] **Задача 2: GET /api/tasks — список с фильтрацией, сортировкой, пагинацией**
  - Фильтрация по `category`, `completed` (`'true'`/`'false'` → булева), `priority` (число 1–5)
  - Сортировка по `dueDate`, `priority`, `createdAt`; знак `-` означает убывание (например, `-priority`)
  - Пагинация: `page` и `limit` (по умолчанию 1 и 10); ответ содержит `total`, `page`, `totalPages`

- [x] **Задача 3: GET /api/tasks/:id — задача по ID**
  - `validateId` middleware проверяет корректность числового ID
  - 404 если задача не найдена

- [x] **Задача 4: POST /api/tasks — создание задачи**
  - Проходит через `validateCreateTask` (Joi)
  - Генерируется `id` (автоинкремент) и `uuid`; сохраняется через `writeData()`

- [x] **Задача 5: PUT /api/tasks/:id — обновление задачи**
  - 404 при отсутствии; слияние `updates` с существующей задачей через spread-оператор
  - Обновляет `updatedAt`, сохраняет через `writeData()`

- [x] **Задача 6: PATCH /api/tasks/:id/complete — отметить выполненной**
  - 404 при отсутствии; устанавливает `completed: true` и `updatedAt`

- [x] **Задача 7: DELETE /api/tasks/:id — удаление**
  - 404 при отсутствии; удаляет элемент через `splice()`, сохраняет через `writeData()`

- [x] **Задача 8: GET /api/tasks/stats/summary — статистика**
  - Подсчёт: `total`, `completed`, `pending`, `overdue` (просроченные невыполненные)
  - Распределение по категориям (`byCategory`) и приоритетам (`byPriority`)

- [x] **Задача 9: GET /api/tasks/search/text — полнотекстовый поиск**
  - Параметр `q` (минимум 2 символа), регистронезависимый поиск по `title` и `description`

- [x] **Задача 10: GET / и GET /health в app.js**
  - `GET /` — возвращает информацию об API (название, версия, эндпоинты)
  - `GET /health` — возвращает `{ status: 'healthy', timestamp }`
...
#### 4. Какие HTTP статус-коды использовались и почему?
...
#### 8. Как бы вы улучшили это API для production?
